...
# Load Dataset
dim_articles = pd.read_csv('resources/train_dataset/dim_articles.csv', header=None)
...
true_articles = dim_articles
fragments = []
for i in range(12):
    sampling = dim_articles.sample(n=30300, random_state=369)
...
#test_dim = dim_articles[0][:32839] # TODO: need shuffle
ms_set = []
for i in range(13):
    _set = set()
    for text in fragments[i].values:        
        _set.update(misspell_i(str(text),i))
    ms_set.append(list(_set))
    

# Sampling misspell sets to make false articles df
false_df = []
for mss in ms_set:
    _df = pd.DataFrame(mss)
    #sampling = _df.sample(n=30000)
    sampling = _df
    false_df.append(sampling)
    
    
# Clean texts in false articles
false_articles = []
for df in false_df:
    for text in df.values:
        false_articles.append(dc.clean_paragraphs(str(text)))
        
        
# Save true and false articles into csv
false_articles = pd.DataFrame(false_articles)
false_articles = false_articles.sample(frac=1).reset_index(drop=True)
#false_articles.to_csv('resources/train_dataset/false_articles.csv', index=False, header=False)
false_articles.to_csv('resources/train_dataset/false_articles_full.csv', index=False, header=False)
