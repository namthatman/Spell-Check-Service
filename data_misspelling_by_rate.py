...
for i in range(15):
    for text in fragments[i].values:
        ...
        for stext in mstext:
            ...
            ms_rate.append(float(get_rate(str(text), str(stext), i)))
...
false_df = []
for text in ms_set:
    false_df.append(dc.clean_paragraphs(str(text)))


false_articles = pd.DataFrame([false_df, ms_rate])
false_articles = false_articles.T

        
        
# Save true and false articles into csv
false_articles = false_articles.sample(frac=1, random_state=369).reset_index(drop=True)
#false_articles.to_csv('resources/train_dataset/false_articles.csv', index=False, header=False)
false_articles.to_csv('resources/train_dataset/false_articles_full_rate.csv', index=False, header=False)
